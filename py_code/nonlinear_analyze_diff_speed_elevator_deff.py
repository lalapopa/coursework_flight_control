...
for i, val in enumerate(linear_names):
    ...
    time_l, value_linear = get_data_from_csv(
        config.PATH_DATA + config.PATH_DATA_MODEL_DD + fast_model_name
    )
    time_nl, value_nonlinear = get_data_from_csv(
        config.PATH_DATA + config.PATH_DATA_MODEL_DD + slow_model_name
    )
    ...
    axes.plot(
        time_nl,
        value_nonlinear,
        label=r"$\dot{\delta}_{{в}_{max}}=15 \frac{град.}{сек.}$",
        linewidth=2,
    )
    axes.plot(
        time_l,
        value_linear,
        "-.",
        label=r"$\dot{\delta}_{{в}_{max}}=60 \frac{град.}{сек.}$",
        linewidth=2,
    )
    ...
    if three_plots == 0:
        ...
        if validated_name == "delta_elevator":
            ax2 = fig.add_axes([0.465, 0.20, 0.4, 0.3])
        ...
        if sys.argv[-1] == "slide":
            save_path = config.PATH_SAVE + f"model_DD_{validated_name}_s.pgf"
            lgd = axes.legend(
...
time_input, value_input = get_data_from_csv(
    config.PATH_DATA + config.PATH_DATA_MODEL_DD + input_signal_name
)
axes.plot(time_input, value_input, ":", label=r"$\Delta H_{зад}$", linewidth=4)
plt.legend()
plt.grid()
...
if sys.argv[-1] == "slide":
    save_path = config.PATH_SAVE + f"model_DD_{param_name}_s.pgf"
    lgd = axes.legend(
        bbox_to_anchor=(0, -0.12),
        loc="upper left",
        borderpad=0,
