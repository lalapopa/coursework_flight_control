...
data = np.array(
    [[settling_time_lin, settling_time_nonlin], [overshoot_lin, overshoot_nonlin]]
)
df = pd.DataFrame(data, index=[r"$t_{рег},\ с$", r"$\sigma,\ \%$"]).rename(
    columns={0: "Линейная модель", 1: "Нелинейная модель"}
)
