...
for height = H_array 
    ...
    xi_s_H_M = (bar_Y_alpha - bar_M_z_omega_z - bar_M_z_dot_alpha)./(2.*omega_0_H_M);
end


xi_s_max = max(xi_s_H_M, [], 'all');
[r, c] = find(xi_s_H_M==xi_s_max);
H_calc_xi = H_array(r); 
mach_calc_xi = V(r,c)/a(r);  
[r, c] = size(xi_s_H_M);
xi_s_H_M(1, c+1) = xi_s_max;

omega_0_max = max(omega_0_H_M, [], 'all');
[r, c] = size(omega_0_H_M);
omega_0_H_M(1, c+1) = omega_0_max;

to_delete = {'aero_data','height','V', 'q', 'bar_M_z_alpha', 'bar_M_z_omega_z', 'bar_Y_alpha'};
clear(to_delete{:});
